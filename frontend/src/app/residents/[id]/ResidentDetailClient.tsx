...
import { ArrowLeft, Building2, MapPin, Phone, Copy, Check, MessageSquare, Mic, CheckCircle } from "lucide-react";
      ...
      <div className="card-brutal p-6 sm:p-10 bg-white border-3 border-black shadow-brutal-xl">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {/* Voice Cloning Card */}
            <div className="bg-[#0f0f1a] border-2 border-black p-5 shadow-brutal-sm space-y-4 rounded-none">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <span className="badge-brutal bg-calmie-yellow text-black text-[10px]">
                    ✨ NEW — VOICE CLONING
                  </span>
                  <h4 className="font-heading font-black text-base text-white mt-1">
                    Call in Your Own Voice
                  </h4>
                  <p className="text-gray-400 text-xs mt-0.5">
                    Record 30–60 sec of your voice — Calmie AI will call {resident.name.split(" ")[0]} sounding just like you.
                  </p>
                </div>
                {!showVoiceRecorder && !clonedVoiceId && (
                  <button
                    type="button"
                    onClick={() => setShowVoiceRecorder(true)}
                    className="btn-yellow text-xs py-2 px-4 flex items-center justify-center gap-1.5 font-bold shadow-brutal-sm bg-calmie-yellow border-2 border-black text-black hover:bg-yellow-300 transition whitespace-nowrap"
                  >
              ...
              {clonedVoiceId && (
                <div className="bg-green-900/30 border border-green-500/40 rounded-xl p-3 flex items-start gap-3">
                  <CheckCircle className="w-5 h-5 text-green-400 flex-shrink-0 mt-0.5" />
                  <div className="text-xs text-green-300">
                    <p className="font-bold text-green-200">Your voice clone is active.</p>
                    <p>When you schedule a call for {resident.name.split(" ")[0]}, the AI will speak in your voice. Your clone ID: <code className="font-mono bg-black/40 px-1 rounded text-[10px]">{clonedVoiceId.slice(0, 12)}…</code></p>
                    <button onClick={() => { setClonedVoiceId(null); setShowVoiceRecorder(false); }} className="text-green-400 hover:text-green-300 underline mt-1">Remove clone</button>
                  </div>
                </div>
              )}
            </div>
